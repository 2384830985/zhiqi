        ...
        <div v-if="!data.firstFloorid" style="margin-bottom:10px;">无</div> -->
			  ...
			  <el-col :span="12">
				  <div class="grid-content bg-purple-light">
				  		<div class="t-pictureAdd-form-lable">栏目状态：</div>
...
export default {
  ...
  methods:  {
      ...
      this.$confirm('确认保存！, 是否继续?', '提示', {
          ...
          if (arr.id) {
            http(ApiSetting.updateForumForum,{forumForum:arr})  
            .then((res)=>{
          ...
          http(ApiSetting.saveForumForum,{forumForum:arr})  
          .then((res)=>{
            if (res.data.status==1) {
              this.$notify({
                title: '成功',
      ...
      var that = this
          ...
          type: 'warning'
           ...
           http(ApiSetting.updateForumForum,{forumForum:{deleted:1,id:that.data.id}})  
          .then((res)=>{
            if (res.data.status==1) {
              this.$notify({
                title: '成功',
